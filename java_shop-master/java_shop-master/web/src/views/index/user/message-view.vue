          ...
          <div class="notification-item flex-view" v-for="item in msgData" :key="item.id">
            ...
            <div class="content-box">
              <div class="header">
                ...
                <span class="delete" title="删除" @click="handleDelete(item)">×</span>
...
import { userListApi, userDeleteApi } from '/@/api/notice'
...
const handleDelete = (item) => {
  const userId = userStore.user_id
  userDeleteApi({ userId, noticeId: item.id }).then(() => {
    msgData.value = msgData.value.filter((m) => m.id !== item.id)
    message.success('已删除')
  }).catch((err) => {
    console.log(err)
    message.error('删除失败')
