            ...
            <a-row :gutter="24">
              ...
              <a-col span="24">
                <a-form-item label="跳转链接" name="link">
                  <a-input placeholder="请输入 http(s):// 开头链接" v-model:value="modal.form.link"/>
                </a-form-item>
              </a-col>
              <a-col span="24">
                <a-form-item label="广告标语" name="slogan">
                  <a-input placeholder="前台将红色展示该标语" v-model:value="modal.form.slogan"/>
                </a-form-item>
              </a-col>
            </a-row>
...
const modal = reactive({
  ...
  rules: {
    link: [
      ...
      { pattern: /^https?:\/\/.+/i, message: '链接必须以 http:// 或 https:// 开头', trigger: 'change' }
    ],
    slogan: [{required: true, message: '请输入广告标语', trigger: 'change'}],
  },
});
...
const handleOk = () => {
  myform.value?.validate()
      .then(() => {
        ...
        if (modal.form.slogan) {
          formData.append('slogan', modal.form.slogan);
        }
